    ...
    self.getGuidesForDiskIntervention = function () {
        return User.getUser().then((user) => {
            if (user && _.has(constants.urls, [user.ovhSubsidiary, "guides"])) {
                return {
                    diskSerial: constants.urls[user.ovhSubsidiary].guides.diskSerial || constants.urls.FR.guides.diskSerial,
                    noMegaRaidLED: constants.urls[user.ovhSubsidiary].guides.noMegaRaidLED || constants.urls.FR.guides.noMegaRaidLED,
                    megaRaidLED: constants.urls[user.ovhSubsidiary].guides.megaRaidLED || constants.urls.FR.guides.megaRaidLED
                };
            }
            return null;
        });
