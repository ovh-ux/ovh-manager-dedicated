angular.module("Module.otrs.controllers").controller("otrsCtrl", [
    ...
    function ($rootScope, $scope, $location, $timeout, $translate, OtrsPopupService, Otrs, Alerter, constants, OTRS_POPUP_UNIVERSES) {
        ...
        $scope.currentPage = $location.search() && $location.search().currentPage != null ? parseInt($location.search().currentPage, 10) : 1;
        ...
        $scope.$watch(
            ...
            _.debounce(() => {
                ...
                if ($scope.search.subject !== null && $scope.search.subject.length > 0 && $scope.search.subject.length < 3) {
                    return;
                }
                saveSearch();
                $scope.$apply(() => {
        ...
        $scope.transformItem = function (ticket) {
            ...
            return Otrs.getTicket(ticket).then((_ticket) => {
                ...
                if (serviceDescription) {
                    _ticket.serviceDescription = serviceDescription;
                }
                return _ticket;
            });
        };
        ...
        $scope.getServiceDescription = function (ticket) {
            ...
            if ($scope.list.services.length > 0) {
                serviceMap = _.find($scope.list.services, (service) => service.serviceName === ticket.serviceName);
                ...
                .then(
                    (data) => {
                        _.each(data.results, (category) => {
                            ...
                            $scope.list.services.push(
                                ..._.map(category.services, (service) => {
                                    ...
                                    service.serviceDescription = service.displayName;
                                    return service;
                                })
                            );
                        });
                        ...
                        if ($location.search() && $location.search().serviceName) {
                            $scope.search.selectedService = _.find($scope.list.services, (service) => service.serviceName === $location.search().serviceName);
                        }
                    },
                    (err) => {
        ...
        function saveSearch () {
            ...
            if (angular.equals($scope.search, { minCreationDate: null, maxCreationDate: null, serviceName: null, status: null, subject: null })) {
