...
angular.module('Billing.controllers').controller('Billing.controllers.AutoRenew', [
  ...
  function BillingAutoRenewCtrl(
    $filter, $http, $location, $q, $rootScope, $scope, $state, $timeout, $translate, $window,
    Alerter, AutoRenew, billingUrls, constants, PaymentInformation, renewHelper, User,
    AUTORENEW_EVENT, BILLING_BASE_URL, SUBSIDIARIES_WITH_RECENT_AUTORENEW,
  ) {
    /**
    ...
    $scope.gotoExchangeRenew = function gotoExchangeRenew(service) {
    ...
    $scope.gotoRenew = function gotoRenew(service) {
    ...
    $scope.updateServices = function updateServices(serviceName) {
    ...
    $scope
      .verifyUserHasAutoRenewRightOnService = service => userIsBillingOrAdmin(service, $scope.user);
    ...
    $scope.selectedServicesWithoutForcedRenew = function selectedServicesWithoutForcedRenew() {
    ...
    $scope.canDisableAutorenew = function canDisableAutorenew(service) {
      return service.renew
        && !service.renew.deleteAtExpiration
