class PaymentInformationBridge {
  ...
  initPaymentMethod() {
    this.getValidBankAccounts = () => this.PaymentMethod
      .getValid(data => data.filter(paymentMethod => paymentMethod.paymentType === 'bankAccount'));

    this.hasDefaultPaymentMean = () => this.PaymentMethod
      .getValid(data => data.filter(paymentMethod => paymentMethod.default === true))
      .then(paymentMethods => paymentMethods && paymentMethods > 0);

    this.hasDefaultPaymentMean = () => this.PaymentMethod
      .getValid()
      .then(result => result && result.length > 0);

    this.getDetailedPaymentMeans = () => this.PaymentMethod.get()
      .then(paymentMethods => paymentMethods.map(payment => ({
        ...
        getIcon() {
          return this.subtype
            ? _.camelCase(this.subtype).toLowerCase()
            : _.camelCase(this.type).toLowerCase();
        },
      })))
      .then(paymentMethods => ({
    ...
    this.setAsDefaultPaymentMean = mean => this.PaymentMethod.update({
      id: mean.id,
      default: true,
    });
    ...
    this.updatePaymentMean = data => this.PaymentMethod.update({
      id: data.mean.id,
      description: data.description,
    });

    this.deletePaymentMean = id => this.PaymentMethod.remove({ id })
      .then(() => this.$rootScope.$broadcast(this.events.PAYMENT_MEAN_CHANGED));
  ...
  initPaymentMeans() {
    ...
    this.hasDefaultPaymentMean = () => this.PaymentMean.getDetailedPaymentMeans().then(
      paymentMeans => _.chain(paymentMeans)
        .values()
        .flatten()
        .find({ defaultPaymentMean: true, state: 'valid' })
        .value() !== undefined,
    );
    this.hasDefaultPaymentMean = () => this.PaymentMean
      .getValidPaymentMeansIds()
      .then(result => result && result.length > 0);

    this.getDetailedPaymentMeans = () => this.PaymentMean.getDetailedPaymentMeans();

    this.setAsDefaultPaymentMean = (mean, type) => this.PaymentMean
      .setAsDefaultPaymentMean(mean, type);
    ...
    this.deletePaymentMean = (id, paymentType) => this.PaymentMean[`delete${getCamelizedPaymentType(paymentType)}`]('/{id}', {
      urlParams: {
        id,
      },
