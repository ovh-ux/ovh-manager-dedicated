angular.module('Billing.controllers').controller('Billing.controllers.Mean', [
  ...
  'Alerter',
  'PAYMENT_EVENT',
  function ($scope, $q, $log, $translate, baseUrl, BILLING_CONSTANTS, Mean, User, UserApp, Alerter,
    PAYMENT_EVENT) {
        ...
        .then(() => {
          getAllPaymentMeans().forEach((element) => {
            if (element.id !== mean.id) {
              _.set(element, 'defaultPaymentMean', false);
            }
          });
        })
        .catch((err) => {
          _.set(mean, 'defaultPaymentMean', false);
    ...
    $scope.updateMean = function (type, mean, description) {
      ...
      Mean.updatePaymentMean({
        ...
        .then(() => {
          _.set(mean, 'description', description);
        })
        .catch((err) => {
    ...
    function init() {
      const paymentMeansPromise = $scope.getPaymentMeans()
        .then(warnAboutBankAccountsPendingValidation);
