angular.module("Billing.services").service("BillingPayments", [
    ...
    function ($http, $q, $cacheFactory) {
        ...
        this.getPaymentIds = ({ dateFrom, dateTo, sort = false, paymentType = 0 }) => {
            const apiv7Ops = {
        ...
        this.getPayments = (_ids, limit = 0, offset = 0) => {
            ...
            return $http
                ...
                .then((response) => {
                    ...
                    return res.map((item) => item.error ? angular.extend({ error: item.error }, item.value) : item.value);
                });
        };
        ...
        this.getOperationsDetails = (id, billId, orderId) => this.getOperationsIds(id, billId, orderId).then((operationsIds) => $q.all(_.map(operationsIds, (operationId) => this.getOperation(id, billId, operationId))));
