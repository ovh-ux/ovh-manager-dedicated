angular.module("Billing.services").run(["$rootScope", "atInternet", "AUTORENEW_EVENT", "PAYMENT_EVENT", "OVH_ACCOUNT_EVENT", "FIDELITY_EVENT",
    function ($rootScope, atInternet, AUTORENEW_EVENT, PAYMENT_EVENT, OVH_ACCOUNT_EVENT, FIDELITY_EVENT) {
        "use strict";
        ...
        $rootScope.$on(AUTORENEW_EVENT.TERMINATE_AT_EXPIRATION, (event, data) => trackServiceClickEvent(event, data));
        $rootScope.$on(AUTORENEW_EVENT.CANCEL_TERMINATE, (event, data) => trackServiceClickEvent(event, data));
        $rootScope.$on(AUTORENEW_EVENT.ACTIVATE_AUTO_PAYMENT, (event, data) => trackServiceClickEvent(event, data));
        $rootScope.$on(AUTORENEW_EVENT.DEACTIVATE_AUTO_PAYMENT, (event, data) => trackServiceClickEvent(event, data));
