angular.module("Billing.controllers").controller("Billing.controllers.FidelityCreditOrder", ($scope, $rootScope, BillingFidelity, translator, FIDELITY_EVENT) => {
    ...
    $scope.getBC = function () {
        ...
        if (amount) {
            BillingFidelity.creditOrder(amount).then(
                (data) => {
                    $scope.$emit(FIDELITY_EVENT.CREDIT);
    ...
    $scope.checkAmount = function () {
        ...
        amount = isAmount($scope.creditOrder.amount);
        if (amount) {
            $scope.creditOrder.erreurFormat = false;
            ...
            } else if (oldBalance < 0 && (balance < 0 || amount < $scope.creditOrder.min)) {
                $scope.creditOrder.erreurMin = true;
            } else {
                $scope.creditOrder.erreurMin = false;
            }
    ...
    $scope.renewStepValidator = function () {
        return !($scope.creditOrder.erreurMin || $scope.creditOrder.erreurMax || $scope.creditOrder.erreurFormat || !$scope.creditOrder.amount);
    };
