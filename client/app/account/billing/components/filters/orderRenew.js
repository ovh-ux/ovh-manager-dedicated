angular.module("Billing.filters").filter("orderRenew", [
    "translator",
    "$filter",
    ...
    function (translator, $filter) {
        ...
        function translateRenewforCompare (value) {
            if (value.automatic || value.forced) {
                return translator.tr("autorenew_service_renew_auto") + (value.period ? $filter("renewFrequence")(value.period) : "");
            } else if (!value.automatic && !value.forced) {
                return translator.tr("autorenew_service_renew_manuel");
            }
            return value;
        }
