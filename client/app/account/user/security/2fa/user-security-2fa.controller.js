angular.module("UserAccount").controller("UserAccount.controllers.doubleAuth.2fa.enable", [
    ...
    function ($rootScope, $scope, $q, $translate, DoubleAuthBackupCodeService, DoubleAuthSmsService, DoubleAuthTotpService, DoubleAuthU2fService, Alerter) {
         ...
         * @return {Boolean}
         */
        $scope.doesStep1IsValid = () => angular.isString($scope.step1.doubleAuthType) && _.size($scope.step1.doubleAuthType);
        ...
        $scope.doesStep2IsValid = () => {
            ...
            case "sms":
                return angular.isObject($scope.step2.sms.secret) && this.userAccountAdd2faSmsForm && this.userAccountAdd2faSmsForm.$valid;
            case "totp":
                return this.userAccountAdd2faTotpForm && this.userAccountAdd2faTotpForm.$valid;
            case "u2f":
                return this.userAccountAdd2faU2fDescriptionForm && this.userAccountAdd2faU2fDescriptionForm.$valid;
            default:
                return false;
            }
        };
        ...
        $scope.validCode = () => {
            switch ($scope.getDoubleAuthType()) {
            case "sms":
                return DoubleAuthSmsService.validate($scope.step2.sms.secret.id, $scope.step2.sms.code)
                    .then(() => {
                        if (!_.isEmpty($scope.step2.sms.description)) {
                            return DoubleAuthSmsService.edit($scope.step2.sms.secret.id, $scope.step2.sms.description);
                        }
                        return $q.when();
                    })
                    .catch((err) => {
            ...
            case "totp":
                return DoubleAuthTotpService.validate($scope.step2.totp.qrCode.id, $scope.step2.totp.code)
                    .then(() => {
                        if (!_.isEmpty($scope.step2.totp.description)) {
                            return DoubleAuthTotpService.edit($scope.step2.totp.qrCode.id, $scope.step2.totp.description);
                        }
                        return $q.when();
                    })
                    .catch((err) => {
            ...
            case "u2f":
                if (!_.isEmpty($scope.step2.u2f.description)) {
                    return DoubleAuthU2fService.edit($scope.step2.u2f.registerChallenge.u2fId, $scope.step2.u2f.description);
                }
                return $q.when();
            default:
                break;
        ...
        $scope.loadSecondStep = () => {
            ...
            return $q
                ...
                .finally(() => {
                    $scope.step2.totp.isLoading = $scope.step2.u2f.isLoading = false;
                });
        };
         ...
         * @return {Promise}
         */
        $scope.loadFourthStep = () => DoubleAuthBackupCodeService.validate(_.first($scope.step3.backupCode.codes));

        $scope.goToFirstStep = () => {
            $scope.step1.isActive = true;
            $scope.step2.isActive = $scope.step3.isActive = $scope.step4.isActive = false;
        };

        $scope.goToSecondStep = () => {
            $scope.step2.isActive = true;
            $scope.step1.isActive = $scope.step3.isActive = $scope.step4.isActive = false;
        };

        $scope.goToThirdStep = () => {
            $scope.step3.isActive = true;
            $scope.step1.isActive = $scope.step2.isActive = $scope.step4.isActive = false;
        };

        $scope.goToFourthStep = () => {
            $scope.step4.isActive = true;
            $scope.step1.isActive = $scope.step2.isActive = $scope.step3.isActive = false;
        };
