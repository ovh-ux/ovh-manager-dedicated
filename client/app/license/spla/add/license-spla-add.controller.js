angular.module("Module.license.controllers").controller("LicenseSplaAddCtrl", [
    "$scope",
    "License",
    "Alerter",
    ...
    function ($scope, License, Alerter) {
        ...
        $scope.load = () => {
            License.splaAddAvailableServers().then(
                ...
                (data) => {
                    $scope.resetAction();
                    Alerter.alertFromSWS($scope.tr("license_spla_add_step1_loading_error"), data.data);
                }
            );
        };
        ...
        $scope.addSpla = () => {
            ...
            License.splaAdd($scope.model.selected.server, { serialNumber: $scope.model.selected.serial, type: $scope.model.selected.type }).then(
                () => {
                    Alerter.alertFromSWS($scope.tr("license_spla_add_success"), true);
                },
                (data) => {
                    Alerter.alertFromSWS($scope.tr("license_spla_add_fail"), data.data);
                }
            );
        };
