angular.module("Module.license.controllers").controller("LicenseOrderCtrl", ($scope, $timeout, License, $q, User, Alerter, $filter, featureAvailability, LicenseOrder) => {
    ...
    function isDomainNumberMandatory () {
        return $scope.selected && $scope.selected.version && $scope.selected.version.options !== null && $scope.selected.version.options.length > 0;
    }
    ...
    $scope.isSelectionValid = function () {
        let valid = $scope.selected.licenseType !== null && $scope.selected.version !== null && $scope.selected.ip !== null;
        let moreOptions;
    ...
    $scope.selectType = function (type) {
        if (type && type !== $scope.selected.licenseType && $scope.isAvailable(type)) {
            ...
            if (
                ...
                $scope.selected.version = $scope.types[$scope.selected.licenseType].options[0].options[0];
            }
    ...
    $scope.isAvailable = function (type) {
        if ($scope.types) {
            return !!$scope.types[type] && $scope.types[type].options && $scope.types[type].options.length > 0 && (!featureAvailability.allowLicenseAgoraOrder() || featureAvailability.allowLicenseTypeAgoraOrder(type));
        }
