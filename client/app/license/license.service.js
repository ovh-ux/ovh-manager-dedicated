angular.module("Module.license").service("License", [
    ...
    function (Api, constants, $q, types, $rootScope, OvhHttp, featureAvailability) {
        ...
        function hasOption (options, name) {
            return options && options[name] !== undefined && options[name] !== null && options[name].value !== null;
        }

        function setPleskOptions (opts, data) {
            if (data.options) {
                if (hasOption(data.options, "languagePackNumber")) {
                    opts.languagePackNumber = data.options.languagePackNumber.value;
                }

                if (hasOption(data.options, "antivirus")) {
                    opts.antivirus = data.options.antivirus.value;
                }

                if (hasOption(data.options, "domainNumber")) {
                    opts.domainNumber = data.options.domainNumber.value.value;
                }

                if (hasOption(data.options, "powerpack")) {
                    opts.powerpack = data.options.powerpack.value;
                }
