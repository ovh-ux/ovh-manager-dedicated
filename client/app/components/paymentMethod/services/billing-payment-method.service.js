...
angular.module("services").service(
    ...
    class BillingPaymentMethod {
        constructor ($q, $http, $timeout, $httpParamSerializerJQLike, MePaymentMethodApi, BillingVantivInstance, BILLING_PAYMENT_METHOD, BILLING_VANTIV) {
            this.$q = $q;
            this.$http = $http;
        ...
        _filterByValidStatus (paymentMethods) {
            return _.filter(paymentMethods, "status", "VALID");
        }

        _filterOnlyOneWithId (paymentMethods) {
        ...
        _filterNonExcludedPaymentTypes (paymentTypes) {
            return _.filter(paymentTypes, (paymentType) => !_.includes(this.BILLING_PAYMENT_METHOD.EXCLUDED_AUTOMATIC_TYPES_FOR_CREATION, paymentType));
        }
        ...
        getAvailable () {
            return this.MePaymentMethodApi.get()
                .$promise.then(this._filterByValidStatus)
                .then(this._filterOnlyOneWithId);
        }
        ...
        _extractPaymentMethodId (submitUrl) {
        ...
        _submit (submitUrl, newPaymentMethod) {
            return this.$http({
                method: "POST",
                url: submitUrl,
        ...
        _pollStatusChanged (id, timeoutDate, initialStatus = "CREATED") {
            return this._update(id).then((data) => {
                if (data.status === initialStatus) {
                    if (timeoutDate.isAfter(new Date())) {
                        ...
                        return this.$timeout(() => this._pollStatusChanged(id, timeoutDate, initialStatus), interval);
                    }
        ...
        isCreditCardVantivError (error) {
            const errorCode = this._getVantivErrorCode(error);
        ...
        add (paymentMethod) {
            return this._create(paymentMethod).then((data) => {
                const id = data.id;
                const submitUrl = data.submitUrl;
                const paymentMethodId = this._extractPaymentMethodId(submitUrl);

                return this.BillingVantivInstance.submit(`payment_mean_${paymentMethodId}`)
                    .then((response) => this._submit(submitUrl, response))
                    .then(() => {
                        const timeoutDate = moment().add(this.BILLING_PAYMENT_METHOD.UPDATE_POLL.TIMEOUT, "ms");
                        return this._pollStatusChanged(id, timeoutDate);
                    })
                    .then((response) => {
                    ...
                    .catch((error) => this._delete(id).finally(() => this.$q.reject(error)));
            });
        }

        // Those rules came from https://en.wikipedia.org/wiki/Payment_card_number
        _extractCreditCardCompany (cardNumber6And4) {
            ...
            // Valid american_express should begin with a "." since API doesn't return a BIN number for them (only last 4 digits)
            if (_.startsWith(cardNumber6And4, ".")) {
                return this.cardTypes.AMERICAN_EXPRESS;
            }

            if ((firstTwoDigitsNumber >= 51 && firstTwoDigitsNumber <= 55) || (firstFourDigitsNumber >= 2221 && firstFourDigitsNumber <= 2720)) {
        ...
        extractCreditCardDetails (cardNumber6And4) {
            ...
            const cardType = this._extractCreditCardCompany(cardNumber6And4);
