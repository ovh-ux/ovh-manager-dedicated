...
angular.module("filters").filter("bandwidth", [
    "translator",
    function (translator) {
        "use strict";
        ...
        function getValueAndUnitForToUnit (bytes, precision, toUnit) {
            ...
            const unit = translator.tr("unit_size_bps"); // Bytes by default

            if (toUnit !== translator.tr("unit_size_bps")) {
                for (i = 0, ii = unitsValues.length; i < ii; i++) {
                    if (toUnit === translator.tr(`unit_size_${unitsValues[i].unit}`)) {
