angular.module("Module.ip.filters").filter("availableIpText", ($translate) => {
    "use strict";
    ...
    return function (ip) {
        ...
        if (ip.blockSizes.length > 1) {
            ...
            html += ip.number > 1 ? $translate.instant("ip_order_step1_DEDICATED_v2_individual_other", {
                t0: ip.number
            }) : $translate.instant("ip_order_step1_DEDICATED_v2_individual_1");
            if (ip.blockSizes[0] !== 1) {
                html += ` ${$translate.instant("ip_order_step1_DEDICATED_individualblocksize", {
                    t0: ip.blockSizes[0]
                })}`;
            }
        }
