angular.module("Module.ip.controllers").controller("IplbDashboardCtrl", [
    ...
    function ($scope, $rootScope, $q, $location, Iplb, Alerter) {
        "use strict";
        ...
        function init () {
            ...
            return Iplb.getList().then((list) => {
                $scope.iplbList = list;
                if (list && list.length) {
                    if ($location.search().serviceName && ~list.indexOf($location.search().serviceName)) {
                    ...
                    $rootScope.preselectedIplb = $scope.selectedIplb;
                }
                    ...
                    (serviceInfos) => {
                        ...
                        if (serviceInfos.status === "expired") {
                            ...
                            return Iplb.getDetails($scope.selectedIplb.value).then(
                                ...
                                (data) => {
                                    Alerter.alertFromSWS($scope.tr("iplb_infos_error"), data);
                                }
                            );
                        }
                        return null;
        ...
        $scope.selectTabZone = function (value) {
            ...
            const queue = [];
            queue.push(
                Iplb.getInternalNatIp($scope.selectedIplb.value, $scope.selectedIplb.zone.selected).then((internalNatIp) => {
                    $scope.selectedIplb.zone.internalNatIp = internalNatIp;
                })
            );
            queue.push(
                Iplb.getProbeIp($scope.selectedIplb.value, $scope.selectedIplb.zone.selected).then((probeIp) => {
        ...
        function loadPortsRedirections () {
            Iplb.getPortsRedirections($scope.selectedIplb.value).then((portsRedirectionIds) => {
        ...
        $scope.$on("iplb.addBackend.start", (event, backend) => {
            ...
            message += $scope.tr("iplb_backend_add_success");
        ...
        $scope.$on("iplb.delBackend.start", (event, backend) => {
            ...
            message += $scope.tr("iplb_backend_delete_success");
            Alerter.success(message, "polling_action");
        });
        $scope.$on("iplb.backupStateSet.start", (event, backend) => {
            ...
            message += $scope.tr("iplb_backend_setbackupstate_success");
            Alerter.success(message, "polling_action");
        });
        $scope.$on("iplb.backupStateUnset.start", (event, backend) => {
            ...
            message += $scope.tr("iplb_backend_unsetbackupstate_success");
            Alerter.success(message, "polling_action");
        });
        $scope.$on("iplb.setWeight.start", (event, backend) => {
            ...
            message += $scope.tr("iplb_backend_setweight_success");
            Alerter.success(message, "polling_action");
        });
