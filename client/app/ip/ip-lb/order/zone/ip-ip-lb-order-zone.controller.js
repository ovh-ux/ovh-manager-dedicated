angular.module("Module.ip.controllers").controller("IplbOrderPopCtrl", [
    "$scope",
    "$rootScope",
    "$q",
    "Ip",
    ...
    function ($scope, $rootScope, $q, Ip, Iplb, Alerter) {
        "use strict";
        ...
        $scope.getOrder = function () {
            ...
            Iplb.getOrderPop($scope.data.value, $scope.model.pop).then(
                ...
                (data) => {
                    Alerter.alertFromSWS($scope.tr("iplb_pop_order_failure"), data);
                    $scope.resetAction();
                }
            );
        };
