angular.module("Module.ip.controllers").controller("IplbBackendSetBackupStateCtrl", [
    "$scope",
    "$rootScope",
    "Iplb",
    "Alerter",
    ...
    function ($scope, $rootScope, Iplb, Alerter, $q) {
        "use strict";
        ...
        $scope.setBackupState = function () {
            ...
            Iplb.setBackupState($scope.data.selectedIplb.value, $scope.data.backend.backend, $scope.model)
                .then(
                    ...
                    (reason) => {
                        Alerter.alertFromSWS($scope.tr("iplb_backend_backupStateSet_failure"), reason);
                    }
                )
                .finally(() => {
                    $scope.resetAction();
