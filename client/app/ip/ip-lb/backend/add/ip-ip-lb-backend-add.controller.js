angular.module("Module.ip.controllers").controller("IplbBackendAddCtrl", ($scope, $rootScope, $q, $translate, Ip, Iplb, Alerter, IpRange) => {
    ...
    Object.defineProperty($scope, "stepIsValid", {
        ...
        get () {
            ...
            if ($scope.model.weight < $scope.weightConf.MIN || $scope.model.weight > $scope.weightConf.MAX) {
                return false;
            }
    ...
    $scope.$watch("model.ipblockBackend", (val) => {
        $scope.allowedBackendsIps = val ? IpRange.getRangeForIpv4Block(val) : [];
        $scope.model.ipBackend = $scope.allowedBackendsIps.length === 1 ? $scope.allowedBackendsIps[0] : undefined;
    });
