angular.module("Module.ip.services").service("IpVirtualMac", [
    ...
    function ($http, $q, constants, Poll) {
        ...
        this.addVirtualMacToIp = (serviceName, ipAddress, type, virtualMachineName) =>
            $http.post(`${swsProxypassPath}/dedicated/server/${serviceName}/virtualMac`, { ipAddress, type, virtualMachineName }).then((data) => data.data, (http) => $q.reject(http.data));

        this.addIpToVirtualMac = (serviceName, macAddress, ipAddress, virtualMachineName) =>
            $http.post(`${swsProxypassPath}/dedicated/server/${serviceName}/virtualMac/${macAddress}/virtualAddress`, { ipAddress, virtualMachineName }).then((data) => data.data, (http) => $q.reject(http.data));
