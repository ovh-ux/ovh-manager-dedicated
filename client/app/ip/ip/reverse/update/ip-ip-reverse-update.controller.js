angular.module("Module.ip.controllers").controller("IpIpv6ReverseDelegationCtrl", ($scope, $rootScope, $translate, Ip, IpReverse, Alerter, Validator, $q) => {
    ...
    $scope.loadStep2 = function () {
        $scope.reversesToAdd = _.difference($scope.model.reverses, $scope.data.ipBlock.reverseDelegations);
        $scope.reversesToDelete = _.difference($scope.data.ipBlock.reverseDelegations, $scope.model.reverses);
    };
    ...
    $scope.addIpv6ReverseDelegation = function () {
        ...
        const queueToDelete = $scope.reversesToDelete.map((reverse) => IpReverse.deleteDelegation($scope.data.ipBlock.ipBlock, reverse));

        $q.all(queueToDelete).then(
            () => {
                const queueToAdd = $scope.reversesToAdd.map((reverse) => IpReverse.setDelegation($scope.data.ipBlock.ipBlock, reverse));
