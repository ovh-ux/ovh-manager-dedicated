            ...
            IpFirewall.getFirewallRules($scope.selectedBlock, $scope.selectedIp, rulesCount, offset)
                .then(
                    (rules) => {
                        ...
                        if ($scope.rules && $scope.rules.list && $scope.rules.list.results && $scope.rules.list.results.length) {
                            angular.forEach($scope.rules.list.results, (result, i) => {
                                ...
                                if (result.state === "CREATION_PENDING" || result.state === "REMOVAL_PENDING") {
                                    IpFirewall.pollFirewallRule($scope.selectedBlock, $scope.selectedIp, result.sequence).then(() => {
                                        reloadRules();
                                    });
                                }
                            });
