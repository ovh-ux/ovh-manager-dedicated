angular.module("Module.ip.controllers").controller("IpFirewallAddRuleCtrl", ($scope, $rootScope, Ip, IpFirewall, Alerter, Validator) => {
    ...
    IpFirewall.getFirewallRuleConstants().then((constants) => {
        ...
        constants.tcpOptions = _.union(["NONE"], constants.tcpOptions);
    ...
    $scope.isFirewallRuleFormValid = function () {
        ...
        // Required field
        $scope.validator.required = $scope.rule.sequence !== undefined && $scope.rule.action !== undefined && $scope.rule.protocol !== undefined;

        if ($scope.rule.source) {
            $scope.validator.source = (Validator.isValidIpv4($scope.rule.source) && !sourceIp.test($scope.rule.source)) || Validator.isValidIpv4Block($scope.rule.source);
        } else {
        ...
        return $scope.validator.required && $scope.validator.source && $scope.validator.sourcePort && $scope.validator.destinationPort && $scope.validator.fragment;
    };
    ...
    $scope.addRule = function () {
        ...
        $scope.rule.source = _.isEmpty($scope.rule.source.trim()) ? null : $scope.rule.source;
        $scope.rule.sourcePort = _.isEmpty($scope.rule.sourcePort.trim()) ? null : $scope.rule.sourcePort;
        $scope.rule.destinationPort = _.isEmpty($scope.rule.destinationPort.trim()) ? null : $scope.rule.destinationPort;
