angular.module("Module.ip.services").service("Ip", function ($rootScope, $http, $q, constants, IpRange, $location, OvhHttp) {
    ...
    this.getServerModels = () =>
        $http.get(`${swsProxypassPath}/dedicated/server.json`, { cache: true }).then((response) => {
    ...
    this.getIpModels = () =>
        $http.get(`${swsProxypassPath}/ip.json`, { cache: true }).then((response) => {
            if (response && response.data && response.data.models) {
                return response.data.models;
            }
            return {};
    ...
    this.getOrderModels = () =>
        $http.get(`${swsProxypassPath}/order.json`, { cache: true }).then((response) => {
            if (response && response.data && response.data.models) {
                return response.data.models;
            }
            return {};
    ...
    this.getMeModels = () =>
        $http.get(`${swsProxypassPath}/me.json`, { cache: true }).then((response) => {
            if (response && response.data && response.data.models) {
                return response.data.models;
            }
            return {};
    ...
    function getIpsSanitized (ipBlock, _ips = []) {
        ...
        angular.forEach(ips, (ip) => {
            if (!ip.mitigation) {
                ip.mitigation = "DEFAULT";
            }
            if (!ip.firewall) {
                ip.firewall = "NOT_CONFIGURED";
            }
        });
    ...
    //     return $http.get([swsIpPath, serviceType, serviceName].join("/")).then(function (response) {
    ...
    //     switch(serviceType) {
    //     case "DEDICATED":
    //         return $http.get([swsProxypassPath, "dedicated/server", serviceName].join("/")).then(function(response) {
    ...
    //         });
    //     case "PCC":
    //         return $http.get([swsProxypassPath, "dedicatedCloud", serviceName].join("/")).then(function(response) {
    ...
    //         });
    //     case "VRACK":
    //         return $http.get([swsProxypassPath, "vrack", serviceName].join("/")).then(function(response) {
    ...
    this.getIpMove = (block) =>
        $http.get(`${swsProxypassPath}/ip/${window.encodeURIComponent(block)}/move`).then((data) => {
            const destinationIps = _.reduce(
                data.data,
                (concatList, destination, serviceType) => {
                    const obj = destination.map((infos) => ({ service: infos.service, nexthop: infos.nexthop, serviceType }));
