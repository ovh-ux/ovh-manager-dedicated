...
==============================*/
    ...
    $scope.orderFormValid = function () {
        if (!$scope.orderableIp || !$scope.model.service || !$scope.model.service.serviceType || !$scope.model.params) {
            return false;
        }
        switch ($scope.model.service.serviceType) {
        case "DEDICATED":
            if (!$scope.model.params.blockSize || ($scope.orderableIp.isCanadianServer ? $scope.model.params.blockSize === 1 && !$scope.model.params.country : !$scope.model.params.country)) {
                return false;
            } else if ($scope.model.params.blockSize > 1 && !$scope.orderableIp.isCanadianServer) {
                // No orga in CA
...
==============================*/
    ...
    function loadPrices (durations) {
        ...
        angular.forEach(durations, (duration) => {
            queue.push(
                IpOrder.getOrderForDuration($scope.model.service, $scope.model.params, duration).then((details) => {
