angular.module("Module.ip.controllers").controller("IpMigrateController", ($scope, $q, Ip, IpOrder, User, Alerter, $rootScope, Validator) => {
    ...
    $scope.isValidIp = function (ip) {
        if (ip != null) {
            ...
            if (ipslash.length > 1) {
                return (Validator.isValidIpv4(ipslash[0]) && ipslash[1] > 0 && ipslash[1] < 33) || (Validator.isValidIpv6(ipslash[0]) && ipslash[1] > 0 && ipslash[1] < 129);
            }
    ...
    $scope.getServices = function () {
        ...
        queue.push(
            IpOrder.getServicesByType("DEDICATED").then((services) => {
    ...
    $scope.checkServiceMigrateable = function () {
        ...
        IpOrder.checkIfAllowed($scope.migrate.service, "ipMigration").then(
            (serviceAllowed) => {
                if (!serviceAllowed) {
                ...
                IpOrder.getServiceMigrateableIp($scope.migrate).then(
                    (infos) => {
                        if (!infos) {
    ...
    $scope.getDurations = function () {
        ...
        angular.forEach($scope.durations.available, (duration) => {
            queue.push(
                IpOrder.getMigrateIpOrder($scope.migrate, duration).then((details) => {
        ...
        $q.all(queue).then(
            () => {
                if ($scope.durations.available && $scope.durations.available.length === 1) {
                    $scope.migrate.duration = $scope.durations.available[0];
                }
    ...
    $scope.loadContracts = function () {
        ...
        if (!$scope.durations.details[$scope.migrate.duration].contracts || !$scope.durations.details[$scope.migrate.duration].contracts.length) {
