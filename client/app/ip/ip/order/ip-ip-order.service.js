            ...
            case "DEDICATED":
                return $http.get([swsProxypassPath, "dedicated/server", service.serviceName, "orderable/ip"].join("/"))
                    .then((response) => {
                        ...
                        data.ipv4BlockSizesAll = data.ipv4.reduce((globalArray, val) => [...val.blockSizes, ...globalArray], []);
                            ...
                            return $http.get([swsProxypassPath, "vps", service.serviceName, "ips"].join("/")).then(
                                (dataIps) => {
                                    ...
                                    return $q.all(queue).then(
                                        () => ({
                                            maximumAdditionnalIp: data.data.model.maximumAdditionnalIp - selfAdditionalIp,
                                            ipRange: _.range(1, data.data.model.maximumAdditionnalIp - selfAdditionalIp + 1),
                                            vpsInfos: data.data
                                        }),
                                        (http) => $q.reject(http)
                                    );
