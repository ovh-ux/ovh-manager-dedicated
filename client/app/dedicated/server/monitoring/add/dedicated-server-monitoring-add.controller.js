angular.module("App").controller("MonitoringAddCtrl", function ($rootScope, $scope, $stateParams, $translate, Server, Alerter, $q) {
    ...
    function addEMailNotifications (monitoring) {
        const emailNotifications = self.emailNotifications.filter((notification) => notification.email !== null && validator.isEmail(notification.email) && notification.language !== null);
        let promises = [];
    ...
    function addSmsNotifications (monitoring) {
        const smsNotifications = self.smsNotifications.filter(
            (notification) => notification.smsAccount !== null && notification.phoneNumberTo !== null && notification.fromHour !== null && notification.toHour !== null && notification.language !== null
        );
