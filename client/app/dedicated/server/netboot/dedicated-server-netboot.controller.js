angular.module("controllers").controller("controllers.Server.Netboot", ($scope, Server, Alerter, $q, $stateParams) => {
    ...
    function rootDeviceValidation () {
        return regRootDevice1.test($scope.rootDevice.root) || regRootDevice2.test($scope.rootDevice.root) || regRootDevice3.test($scope.rootDevice.root) || regRootDevice4.test($scope.rootDevice.root);
    }
    ...
    $scope.setNetboot = function () {
        ...
        const rootDevice = ($scope.currentNetboot.type === $scope.NETWORK && $scope.rootDevice.root) || $scope.server.rootDevice;
        const netbootType = $scope.currentNetboot.type;

        $scope.loading.setNetboot = true;

        const promiseList = [Server.setNetBoot($stateParams.productId, netbootId, rootDevice, netbootType)];
        if ($scope.currentNetboot.type === $scope.RESCUE) {
            promiseList.push(Server.updateRescueMail($stateParams.productId, netbootId, $scope.currentNetboot.rescueMail));
        }
