angular.module('App').controller('ServerCtrl', (NO_AUTORENEW_COUNTRIES, WEATHERMAP_URL, $scope, $timeout, $stateParams, $translate, Server, Polling, $q, constants, User, ovhUserPref, featureAvailability) => {
  ...
  function loadServer() {
    ...
    $q
      ...
      .then((data) => {
        ...
        server.expiration = moment([expiration.year(), expiration.month(), expiration.date()])
          .toDate();
  ...
  function loadMonitoring() {
    $q
      ...
      .catch(err => $scope.setMessage($translate.instant('server_dashboard_loading_error'), err.data));
  }
    ...
    Server.addTask($stateParams.productId, task, $scope.$id)
      .then((state) => {
        if (Polling.isResolve(state)) {
          ...
          checkInstallationProgress(task); // eslint-disable-line
        }
      })
      .catch((data) => {
