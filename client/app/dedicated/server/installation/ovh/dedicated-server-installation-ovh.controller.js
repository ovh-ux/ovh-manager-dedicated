    ...
    .controller("ServerInstallationOvhCtrl", ($rootScope, $scope, $q, $stateParams, $translate, Server, $filter, Alerter, TEMPLATE_OS_HARDWARE_RAID_ENUM) => {
        ...
        function getHardwareRaidProfile () {
            return Server.getHardwareRaidProfile($stateParams.productId).then((raidProfile) => {
                ...
                if (_.any(_.get(raidProfile, "controllers"))) {
        ...
        $scope.$watch("installation.diskGroup", (newValue) => {
            if (newValue) {
                if (newValue.diskGroupId !== _.get($scope.informations.diskGroups[0] || {}, "diskGroupId")) {
                    $scope.installation.raidSetup = false;
                }
        ...
        $scope.refreshDiskGroupInfos = function (newDiskGroup) {
            ...
            if ($scope.installation.hardwareRaid.availableSpace) {
                ...
                $scope.informations.totalSize = newDiskGroup.raidController !== null ? Math.round(toBytes(newDiskGroup.diskSize) / 1000 / 1000) : Math.round(toBytes(newDiskGroup.diskSize) / 1000 / 1000) * _.get(newDiskGroup, "numberOfDisks", 0);
            }

            const otherDisk = _.find($scope.informations.diskGroups, (diskGroup) => diskGroup.diskGroupId !== newDiskGroup.diskGroupId);
            $scope.informations.otherDisk = _.map(_.compact([otherDisk]), (disk) => ({
        ...
        $scope.displayNewPartition = function () {
            ...
            if (_.contains($scope.constants.partitionTypeList, $scope.constants.warningLogical)) {
            ...
            if ($scope.installation.selectDistribution.family === $scope.constants.warningWindows) {
                if (_.contains($scope.constants.fileSystemList, $scope.constants.warningNTFS)) {
            ...
            } else if (_.contains($scope.constants.fileSystemList, $scope.constants.warningEXT4)) {
            ...
            if (!$scope.errorInst.order) {
                ...
                if ($scope.installation.selectDistribution.family !== $scope.constants.warningWindows) {
                    $scope.errorInst.orderFirst =
                            (hasBoot && firstPartition.mountPoint !== $scope.constants.warningBoot) || (!hasBoot && firstPartition.mountPoint !== $scope.constants.warningRoot && firstPartition.mountPoint !== $scope.constants.warningBoot);
                } else {
        ...
        $scope.validationRaid = function (partition) {
            ...
            if ($scope.installation.nbDiskUse > 1 && !$scope.informations.raidController) {
                $scope.errorInst.raid0 =
                        partition.raid !== $scope.constants.warningRaid1 && partition.raid !== $scope.constants.warningRaid0 && (partition.mountPoint === $scope.constants.warningBoot || partition.mountPoint === $scope.constants.warningRoot);
        ...
        function validationNbDiskUse (nbDisk) {
            ...
            if (nbDisk !== 1) {
                for (index; index--;) {
                    ...
                    if ($scope.informations.nbDisk > 2 && !_.contains(raidList, $scope.installation.partitionSchemeModels[index].raid)) {
        ...
        function validationSizeMax (partition) {
            $scope.errorInst.partitionSize =
                    partition.fileSystem !== $scope.constants.warningZFS && !$scope.installation.selectDistribution.supportsGpt && $scope.getRealDisplaySize({ partition, notDisplay: true, noRaid: true }) > $scope.constants.maxSizePartition;
        ...
        $scope.recalculateAvailableRaidDisks = function () {
            if ($scope.installation.hardwareRaid.controller) {
                ...
                for (let i = minDisks; i < nbOfDisk + 1; i = i + minDisksPerArray) {
                    $scope.informations.hardwareRaid.availableDisks.push(i);
                }
            }
        };
        ...
        function prepareDiskList () {
            ...
            if ($scope.installation.hardwareRaid.arrays === 1) {
                return _.chain($scope.installation.hardwareRaid.controller.disks[0].names)
                    .take($scope.installation.hardwareRaid.disks)
                    .value();
            }
        ...
        $scope.raidIsPersonnalizable = function () {
            return (
                $scope.constants.server.raidController &&
                    _.get($scope.installation, "selectDistribution.hardRaidConfiguration") !== false &&
        ...
        $scope.hasVirtualDesktop = function () {
            return !_.includes(_.get($scope.installation, "selectDistribution.id"), "hyperv");
        };

        $scope.hasLicencedOs = function () {
            return _.find($scope.installation.distributionList, (distribution) => distribution.family === "WINDOWS");
        };
