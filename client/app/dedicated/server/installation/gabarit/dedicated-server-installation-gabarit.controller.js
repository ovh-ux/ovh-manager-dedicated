angular.module("App").controller("ServerInstallationGabaritCtrl", ($rootScope, $scope, $q, Server, $filter, Alerter, $stateParams) => {
    ...
    $scope.loadStep2 = function () {
        ...
        Server.getHighestPriorityPartitionScheme($stateParams.productId, $scope.installation.selectGabarit.id)
            .then((response) => Server.getPartitionSchemeHardwareRaid($stateParams.productId, $scope.installation.selectGabarit.id, response.name))
            .then((response) => {
                if (response) {
    ...
    $scope.install = function () {
        if ($scope.installation.options.saveGabarit) {
            $scope.loader.loading = true;
            Server.putSetGabarit($stateParams.productId, $scope.installation.selectGabarit.id, $scope.installation.options.gabaritNameSave, {
                ...
                () => {
                    $scope.installation.selectGabarit.displayName = angular.copy($scope.installation.options.gabaritNameSave);
                    $scope.installation.selectGabarit.id = angular.copy($scope.installation.options.gabaritNameSave);
                    startInstall();
                },
                (data) => {
                    $scope.loader.loading = false;
