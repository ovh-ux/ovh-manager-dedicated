angular.module("services").service("ServerFirewallAsa", function ($rootScope, $http, constants, Server, OvhHttp, $q) {
    const self = this;
    ...
    this.getInformations = function (serviceName) {
        return OvhHttp.get("/dedicated/server/{serviceName}/features/firewall", {
            ...
            .then((firewall) => {
                firewall.mode = _.snakeCase(firewall.mode).toUpperCase();
                return firewall;
            })
            .catch((err) => {
                if (err.status === 404) {
