    ...
    function ($scope, Server, statisticsScale, $filter, $timeout, $translate) {
        ...
        const realTimeStats = function () {
            ...
            if ($scope.statisticsScalesAvailable && $scope.statisticsScalesAvailable.length > 0) {
                ...
                Server.getMitigationRealTimeStatistics($scope.selectedIpAndBlock.block.ip, $scope.selectedIpAndBlock.ip.ip).then(
                    (data) => {
                        $scope.data = data;
        ...
        $scope.getStatistics = function () {
            if ($scope.model.from && $scope.model.scale) {
                $scope.statsLoading = true;
                Server.getMitigationStatistics($scope.selectedIpAndBlock.block.ip, $scope.selectedIpAndBlock.ip.ip, $scope.model.from.toISOString(), $scope.model.scale).then(
                    (data) => {
                        $scope.data = data;
                        $scope.statsLoading = false;
                    }, //
