angular.module('controllers').controller('controllers.Server.Stats', (
  $scope, $state, $stateParams, $q, Server, $translate, Alerter, BandwidthVrackOrderService,
  ServerTrafficService, ServerOrderTrafficService, featureAvailability,
) => {
  $scope.currentView = {
    value: 'DASHBOARD',
  };
  ...
  $scope.loadBandwidthInformations = function () {
    $q
      .all([
        ...
        Server
          .getBandwidthVrackOption($stateParams.productId)
          .then(bandwidthVrackOption => ($scope.bandwidthVrackOption = bandwidthVrackOption)),
        BandwidthVrackOrderService
          .getOrderableBandwidths($stateParams.productId)
          .then(bandwidthVrackOptions => (
            $scope.bandwidthVrackOrderOptions = bandwidthVrackOptions)),
        ServerTrafficService
          .getTraffic($stateParams.productId)
          .then(traffic => ($scope.traffic = traffic.data)),
        ServerOrderTrafficService
          .getOrderables($stateParams.productId)
          .then(trafficOrderables => ($scope.trafficOrderables = trafficOrderables.data)),
        ServerOrderTrafficService
          .getOption($stateParams.productId)
          .then(trafficOption => ($scope.trafficOption = trafficOption.data)),
      ])
  ...
  $scope.canOrderVrackBandwidth = () => featureAvailability
    .allowDedicatedServerOrderVrackBandwidthOption()
    && !$scope.server.isExpired && $scope.server.canOrderVrackBandwith;
  $scope.canOrderMoreVrackBandwidth = () => !$scope.server.isExpired
    && $scope.server.canOrderVrackBandwith
    && $scope.bandwidthVrackOrderOptions.data.length;

  $scope.canOrderTraffic = () => featureAvailability.allowDedicatedServerOrderTrafficOption()
    && !$scope.server.isExpired
    && $scope.server.canOrderQuota;
  $scope.canOrderMoreTraffic = () => !$scope.server.isExpired
    && $scope.server.canOrderQuota
    && _.get($scope.trafficOrderables, 'length');
    ...
    $q.all(promises)
      .then((stats) => {
        ...
        return Server.getStatistics(
          ...
          $scope.serverStats.networkChoice,
          $scope.serverStats.typeChoice,
          $scope.serverStats.periodeChoice,
        );
      })
      .then((statistics) => {
