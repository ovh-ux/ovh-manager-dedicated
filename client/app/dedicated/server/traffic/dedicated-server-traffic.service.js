class ServerTrafficService {
    ...
    getTraffic (productId) {
        ...
        return this.Server.getBandwidth(productId)
            .then((bandwidth) => {
                const traffic = bandwidth.traffic;

                const hasQuota = _.get(traffic, "outputQuotaSize") !== null || _.get(traffic, "inputQuotaSize") !== null;

                traffic.remainingInputQuotaSize = this.computeRemainingUsage(traffic.inputQuotaSize, traffic.inputQuotaUsed);
                traffic.remainingOutputQuotaSize = this.computeRemainingUsage(traffic.outputQuotaSize, traffic.outputQuotaUsed);
    ...
    computeRemainingUsage (quota, quotaUsed) {
        ...
        const remainingSize = quotaSize.value <= upsizedQuotaUsed.value ? 0 : quotaSize.value - upsizedQuotaUsed.value;
        ...
        const percent = quotaSize.value <= upsizedQuotaUsed.value ? 100 : upsizedQuotaUsed.value / quotaSize.value * 100;
