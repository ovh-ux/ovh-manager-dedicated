angular.module("services").service("Nas", function (Products, $http, $q, constants, $rootScope, Polling, OvhHttp, Poll) {
    ...
    self.getNasType = function (nasId) {
        if (!_.isEmpty(nasId)) {
    ...
    self.getTasknewPath = function (nasId, taskId) {
    ...
    self.getTaskInProgressOf = function (nasId, operation) {
    ...
    // SWS
    self.getNas = function () {
        return OvhHttp.get("/sws/dedicated/nastype", {
            rootPath: "2api"
        });
    };

    self.getUrlRenew = function (nasId) {
        return URI.expand(constants.renew, {
            serviceName: self.getNasId(nasId)
        }).toString();
    };

    self.getConstant = function (nasId) {
    ...
    self.getSelected = function (nasId, forceRefresh) {
    ...
    self.getServiceInfos = function (nasId, forceRefresh) {
    ...
    self.updateNasDetails = function (nasId, customName, monitored) {
    ...
    self.getPartitionsIds = function (nasId, forceRefresh) {
    ...
    self.addPartition = function (nasId, partitionName, protocol, size) {
    ...
    self.getPartition = function (nasId, partitionName) {
    ...
    self.deletePartition = function (nasId, partitionName) {
    ...
    self.updatePartitionSize = function (nasId, partitionName, size) {
    ...
    self.getAccessIds = function (nasId, partitionName, forceRefresh) {
        return OvhHttp.get("/dedicated/{nasType}/{serviceName}/partition/{partitionName}/access", {
            ...
            clearAllCache: forceRefresh ? [cacheNas.partitionsAccessIds, cacheNas.partitionsAccessDetails] : false
        });
    };

    self.addAccess = function (nasId, partitionName, ip) {
    ...
    self.deleteAccess = function (nasId, partitionName, ip) {
    ...
    // SWS
    self.getAuthorizableIps = function (nasId, partitionName) {
    ...
    self.getSnapshots = function (nasId, partitionName) {
        const promiseTab = [];
    ...
    self.postSnapshots = function (nasId, partitionName, snapshots) {
        const promiseTab = [];

        angular.forEach(snapshots, (snapshot) => {
            if (snapshot.active) {
                promiseTab.push(self.postSnapshot(nasId, partitionName, snapshot.type).catch((data) => $q.reject({ type: snapshot.type, error: data })));
            } else {
                promiseTab.push(self.deleteSnapshot(nasId, partitionName, snapshot.type).catch((data) => $q.reject({ type: snapshot.type, error: data })));
            }
        });

        return $q.allSettled(promiseTab).catch((data) => $q.reject(_.filter(data, (snapshotError) => angular.isDefined(snapshotError.error))));
    };

    self.postSnapshot = function (nasId, partitionName, snapshotType) {
    ...
    self.deleteSnapshot = function (nasId, partitionName, snapshotType) {
    ...
    self.getBCNasha = function (model, datacenter, duration) {
    ...
    self.postBCNasha = function (model, datacenter, duration) {
