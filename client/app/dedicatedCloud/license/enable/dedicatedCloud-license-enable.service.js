...
angular
  .module('App')
  .service('DedicatedCloudLicencesSplaEnableService', class {
    constructor(
      OvhApiOrder,
    ) {
    ...
    fetchContracts(offer, serviceName, ovhSubsidiary) {
      return this.OvhApiOrder.Cart().v6()
        ...
        .then(({ cartId }) => this.OvhApiOrder.Cart().v6()
          .assign({
            cartId,
          }).$promise
          .then(() => this.OvhApiOrder.CartServiceOption().v6().post({
          ...
          .then(() => this.OvhApiOrder.Cart().v6().getCheckout({
            cartId,
          }).$promise))
