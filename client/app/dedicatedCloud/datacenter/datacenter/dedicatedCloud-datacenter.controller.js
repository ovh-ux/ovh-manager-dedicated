angular.module("App").controller("DedicatedCloudSubDatacenterCtrl", ($scope, $stateParams, $timeout, $location, DedicatedCloud, translator) => {
    ...
    $scope.setAction = function (action, data, parent) {
        if (action) {
            ...
            if (parent) {
                ...
                $scope.stepPath = `dedicatedCloud/datacenter/${$scope.currentAction}.html`;
            }
    ...
    /* ===============================================================================
    =            TABS (COPIED FROM OLD DedicatedCloudSubDatacenterTabsCtrl)          =
    ================================================================================ */

    $scope.toKebabCase = _.kebabCase;
    $scope.dedicatedCloud = angular.extend($scope.dedicatedCloud || {}, {
        tabs: ["dashboard", "host", "datastore", "backup", "license"]
    });
    const defaultTab = _.first($scope.dedicatedCloud.tabs);
    ...
    if ($stateParams.tab && ~$scope.dedicatedCloud.tabs.indexOf(angular.uppercase($stateParams.tab))) {
        $scope.setSelectedTab(angular.uppercase($stateParams.tab));
    } else {
        $scope.setSelectedTab(defaultTab);
    }

    /* -----  End of TABS (COPIED FROM OLD DedicatedCloudSubDatacenterTabsCtrl)  ------ */
