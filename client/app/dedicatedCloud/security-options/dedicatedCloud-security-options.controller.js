angular.module("App").controller("DedicatedCloudSecurityOptionsCtrl", ($q, $stateParams, $scope, $translate, Alerter, constants, DedicatedCloud, User, Poller, DEDICATED_CLOUD_CONSTANTS) => {
    ...
    function init () {
        ...
        function loadOptionsStates () {
            return $q.all(
                $scope.optionNames.map((optionName) =>
                    DedicatedCloud.getOptionState(optionName, $stateParams.productId).then((state) => {
    ...
    function pollTask (taskId) {
        return DedicatedCloud.getSelected($stateParams.productId).then((pcc) =>
            Poller.poll(`apiv6/dedicatedCloud/${pcc.name}/task/${taskId}`, null, {
                successRule (task) {
    ...
    function getGuide () {
        User.getUser().then((user) => {
            ...
            $scope.optionNames.forEach((optionName) => {
                _.set($scope.guides, optionName, constants.urls[user.ovhSubsidiary].guides[optionName] || constants.urls[user.ovhSubsidiary].guides.pcidssHdsHipaa || constants.urls.FR.guides.pcidssHdsHipaa);
            });
        });
    }
});
