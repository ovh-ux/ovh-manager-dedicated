angular
  ...
  .controller('DedicatedCloudCtrl', class {
    ...
    $onInit() {
      ...
      this.$scope.$on('ovhAlert.show', (event, type, message, details, alertId) => {
    ...
    showNotificationIfRequired(notification) {
      return this.DucNotification
        .checkIfStopNotification(
          notification,
          this.$transition$.params().productId,
        )
    ...
    loadNewPrices() {
      return this.DedicatedCloud
        .getNewPrices(this.$transition$.params().productId)
        .then((newPrices) => {
    ...
    loadUserInfo() {
      this.$scope.dedicatedCloud.email = this.currentUser.email;
    }
    ...
    loadDedicatedCloud() {
      ...
      return this.DedicatedCloud
        ...
        .then(() => this.DedicatedCloud.getDescription(this.$transition$.params().productId))
        ...
        .then(() => this.OvhApiDedicatedCloud
          .v6()
          .getServiceInfos({ serviceName: this.$transition$.params().productId })
          .$promise)
      ...
      const modal = this.$uibModal.open({
        ...
        resolve: {
          data: () => ({
            ...
            productId: this.$transition$.params().productId,
            value,
          }),
        },
      });
